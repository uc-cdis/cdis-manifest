#!/bin/bash
#
# Custom KF cron job - publishes to QA dashboard.
#
...
# 3   3   *   *   *    (if [ -f $HOME/cdis-manifest/data.kidsfirstdrc.org/reports-cronjob.sh ]; then bash $HOME/cdis-manifest/data.kidsfirstdrc.org/reports-cronjob.sh "vpc=$vpc_name"; else echo "no reports-cronjob.sh"; fi) > $HOME/reports-cronjob.log 2>&1
...
for name in "rtimes-${dateTime}.json" "codes-${dateTime}.json" "users-${dateTime}.json" "projects-${dateTime}.json"; do
  aws --profile gen3qa s3 cp "./$name" "s3://dashboard-222487244010-kfqa2-gen3/gen3qa.kidsfirstdrc.org/Secure/${destFolder}/$name"
done
cd /tmp
