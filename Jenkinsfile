    ...
    stage('DetectChanges') {
      steps {
        script {
          ...
          env.KUBECTL_NAMESPACE = null;
              
          for (int i = 0; i < changeLogSets.size() && env.KUBECTL_NAMESPACE == null; i++) {
            def entries = changeLogSets[i].items
            for (int j = 0; j < entries.length && env.KUBECTL_NAMESPACE == null; j++) {
              ...
              if (affectedPaths.contains('nci-crdc.datacommons.io/manifest.json')) {
                ...
                env.KUBECTL_NAMESPACE = 'jenkins-dcp'
              }
            }
          }
          if (null == env.KUBECTL_NAMESPACE) {
