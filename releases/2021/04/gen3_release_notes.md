...
#### Improvements
  ...
  - Adds support for Typescript to the portal codebase. (#803)
...
#### Deployment Changes
  ...
  - To deploy Discovery page: (#803)
  - `"featureFlags": { "discovery": true }` must be set in the portal config. 
    (#803)
  - If `discoveryConfig.features.authorization.enabled: true`, 
    `"useArboristUI": true` must be set in the portal config. (#803)
  - `"discoveryConfig": <DiscoveryConfig>` must be set in the portal config. 
    DiscoveryConfig documentation can be found at 
    https://github.com/uc-cdis/data-portal/blob/master/docs/portal_config.md 
    (#803)
  - To upload data to Discovery page, data must be uploaded to environment's 
    metadata-service instance with `guid_type: "discovery_metadata"`. The 
    metadata must all be under the top-level property `gen3_discovery`. (No 
    documentation on this process yet, see this gist for right now 
    https://gist.github.com/Avantol13/a53593837503fede2881f315198247c4) (#803)
...
#### New Features
  ...
  - Adds a beta version of the Discovery page, which shows a list of the 
    studies available on the Gen3 commons. (#803)
...
#### # Configuration #
  ...
  - "featureFlags": { "discovery": true } must be set in the portal config. 
    (#803)
  - If discoveryConfig.features.authorization.enabled: true, "useArboristUI": 
    true must be set in the portal config. (#803)
  - "discoveryConfig": <DiscoveryConfig> must be set in the portal config. 
    DiscoveryConfig documentation can be found at 
    https://github.com/uc-cdis/data-portal/blob/master/docs/portal_config.md 
    (#803)
